...
json_web_token::jwt_base64_encode(const string& text) {
  ...
  auto crit = jwt_text.crbegin();
  while (crit != jwt_text.crend() && *crit == '=')
    ++crit;
  jwt_text.resize(jwt_text.size() - (crit - jwt_text.rbegin()));
...
json_web_token::verify(const string& secret_key) const {
  const string msg{jwt_base64_encode(header) + '.' + jwt_base64_encode(payload)};
  const vector<unsigned char> mac_hash{ssl_hmac_hash(get_hash_algorithm(), secret_key, msg)};
